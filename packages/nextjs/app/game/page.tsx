...
// Damage constants
const DAMAGE_MULTIPLIER = 3; // How much damage per unit of impact speed
const MIN_IMPACT_FOR_DAMAGE = 2; // Minimum speed to cause damage
...
const getHitZone = (car: Car, collisionAngle: number): "front" | "rear" | "left" | "right" => {
  // Normalize the collision angle relative to car's facing direction
...
// Simple circle collision check (using car's approximate radius)
const checkCarCollision = (car1: Car, car2: Car): boolean => {
  const dx = car2.x - car1.x;
  const dy = car2.y - car1.y;
  const distance = Math.sqrt(dx * dx + dy * dy);
  const minDistance = ((car1.width + car2.width) / 2) * 0.8; // Slightly smaller for better feel
...
const Game: NextPage = () => {
  ...
  const targetCarRef = useRef<Car>(createCar(CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2, "#3498db", Math.PI / 4, false)); // Not static - can be pushed!
  ...
  // Handle car-to-car collision - ALWAYS separates cars, applies damage on cooldown
  const handleCarCollision = useCallback((playerCar: Car, targetCar: Car) => {
    ...
    // Collision normal (direction from player to target)
    const nx = dx / distance;
    const ny = dy / distance;

    // ALWAYS separate the cars first - this prevents driving through
    const overlap = minDist - distance;
    const separationForce = overlap + 2; // Extra push to prevent sticking

    // Move both cars apart (player moves more since they're the aggressor)
    ...
    // Apply bounce/momentum transfer
    const bounce = 0.7;
    const impactComponent = relVx * nx + relVy * ny;

    if (impactComponent > 0) {
      // Player hitting target - transfer momentum
      ...
      // Target hitting player (or pushing back)
    ...
    // Handle damage (with cooldown to prevent spam)
    const now = Date.now();
    if (now - lastCollisionRef.current < 150) return; // Damage cooldown
    ...
    if (impactSpeed > MIN_IMPACT_FOR_DAMAGE) {
      ...
      playerCar.health[playerHitZone] = Math.max(0, playerCar.health[playerHitZone] - damage * 0.5);
  ...
  const updatePhysics = useCallback(() => {
    ...
    // ===== TARGET CAR PHYSICS =====
    // Apply friction to target car (it slides after being hit)
    ...
    // Apply friction (target car is heavier, more friction)
    ...
    // Wall collisions for PLAYER car
    ...
    // Wall collisions for TARGET car
    ...
    if (targetCollided) {
      targetCar.angularVel += (Math.random() - 0.5) * 0.08;
    }
  ...
  const drawCar = useCallback((ctx: CanvasRenderingContext2D, car: Car) => {
    ...
    // Car body - color based on health
    ...
    // Damage indicators on each zone
    ...
    // Draw damage on each zone
    const hw = car.width / 2;
    const hh = car.height / 2;
    drawDamageZone(hw - 12, -hh, 12, car.height, car.health.front); // Front
    drawDamageZone(-hw, -hh, 12, car.height, car.health.rear); // Rear
    drawDamageZone(-hw, -hh, car.width, 8, car.health.left); // Left (top in car space)
    drawDamageZone(-hw, hh - 8, car.width, 8, car.health.right); // Right (bottom in car space)
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 p-4">
      ...
      <p className="text-gray-400 mb-4">Demolition Derby - Damage Test</p>
      ...
      <div className="mt-4 text-gray-400 text-center">
        ...
        <p className="text-xs mt-1">
          Different zones (Front/Rear/Left/Right) take damage independently. Front hits do most damage!
        </p>
      </div>
